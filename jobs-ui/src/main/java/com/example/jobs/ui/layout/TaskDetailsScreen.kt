...
import com.example.jobs.ui.activity.ui.theme.NavigationAppTheme
...
fun TaskDetailsScreen(modifier: Modifier = Modifier, jobId: Int) {
    ...
    val taskLoadState by tasksViewModel.taskLoadStatus.collectAsStateWithLifecycle()
...
fun TaskDetailsLoadStatesLayout(
    ...
    when(taskLoadState) {
        ...
        is Result.Error -> {
            ErrorScreen(modifier = modifier, message = taskLoadState.error.message ?: "")
        }
        is Result.Success -> {
